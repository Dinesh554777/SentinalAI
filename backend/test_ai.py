...
print("Summary:", data["ai_analysis"].get("summary", "N/A")[:400])
print("Actions:", data["ai_analysis"].get("recommended_actions", []))
print("IOCs:", data["ai_analysis"].get("iocs", []))

print("\n=== Security Summary ===")
r = requests.get(f"{BASE}/ai/summary", headers=headers)
data = r.json()
print("AI Enabled:", data["ai_enabled"])
print("Executive Summary:", data["ai_summary"].get("executive_summary", "N/A")[:400])
...
data = r.json()
print("Risk:", data["risk"], "-", data["risk_score"], "/100")
print("AI Narrative:", data.get("ai_narrative", "N/A")[:400])

print("\n=== Log Analysis ===")
r = requests.post(f"{BASE}/ai/log-analysis?limit=5", headers=headers)
data = r.json()
print("Logs Analyzed:", data["logs_analyzed"])
print("Assessment:", data["analysis"].get("overall_assessment", "N/A")[:300])
print("Threat Level:", data["analysis"].get("threat_level", "N/A"))

print("\n=== All Groq AI endpoints working ===")
